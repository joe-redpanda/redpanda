...
class SegmentMetaAnomaly:
    ...
    def from_dict(json_dict: dict):
        return SegmentMetaAnomaly(
            anomaly_type=json_dict["type"],
            explanation=json_dict["explanation"],
            at_segment=SegmentMeta.from_dict(json_dict["at_segment"]),
            previous_segment=SegmentMeta.from_dict(json_dict["previous_segment"]),
        )
...
class OffsetRange:
    ...
    def overlaps(self, other: "OffsetRange") -> bool:
        return (
            self.first_inclusive <= other.last_inclusive
...
class Anomalies:
    ...
    def from_dict(json_dict: dict) -> Tuple[NTPR, "Anomalies"]:
        ...
        meta_anomalies = set(
            map(
                lambda a: SegmentMetaAnomaly.from_dict(a),
                json_dict.get("segment_metadata_anomalies", []),
            )
        )

        return ntpr, Anomalies(
            ...
            last_complete_scrub_at=json_dict.get("last_complete_scrub_at", None),
        )

    def is_empty(self) -> bool:
        return (
            self.missing_partition_manifest == False
    ...
    def is_subset_of(self, other: "Anomalies") -> bool:
        ...
        return (
            self.missing_segments.issubset(other.missing_segments)
    ...
    def get_impacted_offset_ranges(
        self, ntpr: NTPR, view: BucketView
    ) -> list[OffsetRange]:
        ...
        for spill in self.missing_spillover_manifests:
            ...
            impacted_ranges.append(
                OffsetRange(first_inclusive=meta.base, last_inclusive=meta.last)
            )

        for seg in self.missing_segments:
            obj_meta = ObjectMetadata(key=seg, bucket="", etag="", content_length=0)
...
class CloudStorageScrubberTest(RedpandaTest):
    ...
    def _produce(self):
        ...
        wait_until(
            all_partitions_spilled,
            timeout_sec=60,
            backoff_sec=10,
            err_msg="Some or all partitions did not spill",
        )

    def _query_anomalies(self, admin, namespace: str, topic: str, partition: int):
        def query():
            try:
                res = admin.get_cloud_storage_anomalies(
    ...
    def _collect_anomalies(self) -> dict[NTPR, Anomalies]:
        ...
        for pid in range(self.partition_count):
            json_anomalies = self._query_anomalies(
                admin, namespace="kafka", topic=self.topic, partition=pid
            )
    ...
    def _is_subset_for_all(
        self, superset: dict[NTPR, Anomalies], subset: dict[NTPR, Anomalies]
    ):
    ...
    def _delete_segment_and_await_anomaly(
        ...
        while True:
            ...
            if seg_meta := view.find_segment_in_manifests(to_delete):
                ...
                impacted_ranges = expected_anomalies[ntpr].get_impacted_offset_ranges(
                    ntpr, view
                )
        ...
        self.cloud_storage_client.delete_object(
            self.bucket_name, to_delete.key, verify=True
        )
        ...
        self.expected_error_logs.append(
            re.compile(rf"Can't apply spillover_cmd.*base_offset: {expected_offset}")
        )
    ...
    def _delete_spillover_manifest_and_await_anomaly(
        ...
        while to_delete is None:
            ...
            selected_range = OffsetRange(
                first_inclusive=to_delete.base, last_inclusive=to_delete.last
            )
            impacted_ranges = expected_anomalies[ntpr].get_impacted_offset_ranges(
                ntpr, view
            )
            ...
            attempts += 1
            assert attempts < 100, (
                "Too many attempts to find a spillover manifest to delete"
            )

        self.logger.info(f"Deleting spillover manifest at {to_delete.path}")
        self.cloud_storage_client.delete_object(
            self.bucket_name, to_delete.path, verify=True
        )
        ...
        def ntpr_missing_spill_manifest():
            ...
            found = (
                to_delete.path in anomalies_per_ntpr[ntpr].missing_spillover_manifests
            )
    ...
    def _assert_anomalies_stable_after_leader_shuffle(
        ...
        assert any([a is not None for a in old_anomalies.values()]), (
        ...
        for pid in range(self.partition_count):
            admin.transfer_leadership_to(
                namespace="kafka", topic=self.topic, partition=pid
            )
        ...
        def anomalies_stable():
            ...
            return self._is_subset_for_all(
                superset=anomalies, subset=expected_anomalies
            )
    ...
    def _assert_anomalies_stable_after_restart(
        ...
        self.logger.info("Checking anomalies stay stable after full cluster restart")
    ...
    def _assert_segment_metadata_anomalies(
        ...
        self.logger.info("Fudging manifest and waiting on segment metadata anomalies")
        ...
        assert len(sorted_segments) > 2, (
            f"Not enough segments in manifest: {json.dumps(manifest)}"
        )
        ...
        for crnt_seg, next_seg in itertools.pairwise(sorted_segments):
            ...
            ntpr = NTPR(
                ns="kafka", topic=self.topic, partition=0, revision=meta["ntp_revision"]
            )
            ...
            offset_range = OffsetRange(
                first_inclusive=meta["base_offset"],
        ...
        self.expected_error_logs.append(
            re.compile(
                rf"Can't apply spillover_cmd.*base_offset: {expect_error_at_offset}"
            )
        )
        ...
        def gap_reported():
            ...
            seg_meta_anomalies = anomalies_per_ntpr[ntpr].segment_metadata_anomalies

            for meta in seg_meta_anomalies:
                if (
                    meta.anomaly_type == "offset_gap"
                    and meta.at_segment.base_offset == detect_at_seg_meta["base_offset"]
                ):
                    return True, meta
    ...
    def _assert_metrics_updated(self):
        ...
        def validate_labels(labels):
            label_validators = {
                ...
                "redpanda_topic": [],
                "redpanda_type": [],
                "redpanda_severity": [lambda val: val == "high" or val == "low"],
            }
            ...
            assert expected_labels == found_labels, (
                f"Label keys do not match: {expected_labels=} != {found_labels=}"
            )
        ...
        for node_metrics in metrics:
            for family in node_metrics:
                for sample in family.samples:
                    if (
                        "redpanda_type" in sample.labels
                        and "redpanda_severity" in sample.labels
                    ):
    ...
    def _metrics_present(self, pattern: str, endpoint: MetricsEndpoint) -> bool:
        return (
            self.redpanda.metrics_sample(pattern, metrics_endpoint=endpoint) is not None
        )
    ...
    def test_no_metrics_if_scrubber_disabled(self):
        ...
        self.redpanda.set_cluster_config({"cloud_storage_enable_scrubbing": False})
        ...
        self.redpanda.set_cluster_config({"cloud_storage_enable_scrubbing": True})

        wait_until(
            lambda: self._metrics_present("anomalies", MetricsEndpoint.PUBLIC_METRICS),
            timeout_sec=60,
            backoff_sec=5,
