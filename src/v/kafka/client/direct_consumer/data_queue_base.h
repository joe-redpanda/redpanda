...
struct data_queue_stm {
    ...
    bool can_push(const entry& entry) const;
    bool can_pop(const entry& entry) const;

    void on_push(const entry& entry);
    void on_pop(const entry& entry);
};

template<typename E, typename T, >
concept IsDataQueueSTM = requires(T stm, const E& entry, const T& cstm) {
    { cstm.can_push(entry) } -> std::same_as<bool>;
    { cstm.can_pop(entry) } -> std::same_as<bool>;
    { stm.on_push(entry) } -> std::same_as<void>;
    { stm.on_pop(entry) } -> std::same_as<void>;
};

template<
  typename Entry,
  IsDataQueueSTM<STM> class data_queue_base
  requires IsDataQueueSTM<STM, Entry>
  {
    bool can_insert(const entry& entry) { retu }
}
