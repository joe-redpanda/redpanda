...
public:
    ...
    void produce_to_partition(
      const model::topic& topic, int partition, size_t record_count) {
        ...
        logger.info("producing to ntp: {}", ntp);
    ...
    ss::future<> shuffle_leadership(model::ntp ntp) {
        logger.info("starting the shuffle on ntp: {}", ntp);
        vassert(
          !instance_ids().empty(),
          "broker instances is not allowed to be empty");
        logger.info("ids validated");

        // auto leader_broker = get_partition_leader(ntp);
        auto [rp_ptr, partition_ptr] = get_leader(ntp);
        logger.info("got leader pointers");

        vassert(
          partition_ptr->raft(),
          "must have a valid consensus to transfer leadership");

        logger.info("raft instances are good");
        auto transfer_ec = partition_ptr
                             ->transfer_leadership(
                               {.group = partition_ptr->raft()->group()})
                             .get();

        logger.info("transfer completed with ec: {}", transfer_ec);
        vassert(
          !transfer_ec, "leadership transfer failed with ec: {}", transfer_ec);
...
TEST_P(basic_consume_fixture, TestBasicLeadershipTransfer) {
    assign_partitions(make_assignment(topic, {0, 1, 2}));
    ...
    logger.info("getting ready to shuffle");

    model::ntp to_shuffle{
      model::kafka_namespace, topic, model::partition_id{0}};

    logger.info("shuffling this one {}", to_shuffle);

    this->shuffle_leadership(to_shuffle).get();
